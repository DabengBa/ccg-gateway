...
- D4（已确认）：鉴权必须覆盖「管理 API（/admin/v1/*）+ 网关转发入口（/{path:path}）」。
- D5（已确认）：鉴权方式采用「单个共享 Token」（Header Token）。
- D6（已确认）：允许公网直连后端端口（默认 7788），但仍需应用层鉴权；建议配套限流/防火墙策略。
...
- （已全部确认）详见 0.1 的 D4-D6 与 10.2 最终确认。
...
- MVP（1 句话闭环）：
  - 通过 docker compose 启动后，用户可以在浏览器打开 UI，且所有管理接口与转发接口均需要携带访问凭证（共享 Token）才能使用；同时支持公网直连后端端口调用转发。
- 明确不做（Not in scope）：
...
- [ ] 鉴权（MVP）
  ...
  - [ ] 凭证形态：单个共享 Token（Header Token）。
    - 推荐请求头：X-CCG-Token: <token>（与上游 Authorization 分离，避免混淆/覆盖）。
    - 规则：X-CCG-Token 仅用于客户端->网关鉴权，不得向上游透传。
...
- 接口：
  - /health：健康检查（建议默认无需鉴权，以便探活）。
  - /admin/v1/*：管理 API（必须鉴权）。
  - /{path:path}：转发入口（必须鉴权）。
...
### 10.2 最终确认（Final Decisions）

- 鉴权覆盖范围：/admin/v1/* + /{path:path}（必须鉴权）。
- 鉴权方式：单个共享 Token（Header Token，推荐 X-CCG-Token）。
- 对外端口策略：允许公网直连后端端口（默认 7788），并可选提供 Nginx 反代入口；两种入口均必须鉴权。
